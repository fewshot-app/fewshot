...
/// <summary>
/// Exports a project's memories, preferences, and anti-patterns as an StarkTracePack.
/// </summary>
public class PackExportService
{
    ...
    public PackExportService(StarkTraceDbContext db, ILogger<PackExportService> log)
    {
        _db = db;
        _log = log;
    }
    ...
    /// <summary>
    /// Export all knowledge for a project as an unencrypted StarkTracePack.
    /// </summary>
    public async Task<StarkTracePack?> ExportAsync(string projectName, string? author = null)
    {
        ...
        var pack = new StarkTracePack
        {
            PackId = $"{projectName}-{DateTime.UtcNow:yyyyMMdd}",
            Name = $"{project.DisplayName} Pack",
            Description = $"Exported from StarkTrace — {memories.Count} memories, {preferences.Count} preferences, {antiPatterns.Count} anti-patterns",
            Version = "1.0.0",
