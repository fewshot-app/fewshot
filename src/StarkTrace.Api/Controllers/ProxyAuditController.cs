...
/// <summary>
/// Receives audit events from StarkTrace.Proxy (and potentially other sources).
...
public class ProxyAuditController : ControllerBase
{
    ...
    public ProxyAuditController(StarkTraceDbContext db, ILogger<ProxyAuditController> logger)
    {
        _db = db;
        _logger = logger;
    }
    ...
    public async Task<IActionResult> Log([FromBody] ProxyAuditEvent evt, CancellationToken ct)
    {
        ...
        _db.ProxyAuditLogs.Add(new ProxyAuditLog
        {
            ...
            Source = evt.Source ?? "starktrace-proxy",
