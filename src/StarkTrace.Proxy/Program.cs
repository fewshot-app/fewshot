...
// Intercepts every JSON-RPC message over stdio, scans for PII/secrets, logs to StarkTrace API.
//
...
{
    Console.Error.WriteLine("[StarkTrace-PROXY] Usage: StarkTrace.Proxy.exe --server <mcp-server.exe> [args...]");
    return 1;
}
...
using var server = Process.Start(psi)
    ?? throw new InvalidOperationException($"[StarkTrace-PROXY] Failed to start: {serverExe}");
...
using var http = new HttpClient { BaseAddress = new Uri(starktraceApiBase), Timeout = TimeSpan.FromSeconds(3) };
...
Console.Error.WriteLine($"[StarkTrace-PROXY] Server exited with code {server.ExitCode}");
...
// ═════════════════════════════════════════════════════════════════════════════
// AuditClient — scans JSON-RPC messages, logs findings to StarkTrace API
// ═════════════════════════════════════════════════════════════════════════════
public class AuditClient
{
    ...
    public async Task<string> ProcessAsync(string rawLine, string direction)
    {
        ...
        if (blocking.Count > 0)
        {
            ...
            Console.Error.WriteLine($"[StarkTrace-PROXY] ⚠ REDACTED {direction} ({method}): {types}");
            return Redact(rawLine, blocking);
        }

        var types2 = string.Join(", ", findings.Select(f => f.Type).Distinct());
        Console.Error.WriteLine($"[StarkTrace-PROXY] ℹ Logged {direction} ({method}): {types2}");
        return rawLine;
    }
    ...
    private async Task LogAsync(string direction, string method, List<ProxyFinding> findings, string snippet, bool wasRedacted)
    {
        ...
        catch { /* StarkTrace API unavailable — proxy keeps running */ }
    }
