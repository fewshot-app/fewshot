...
public class PackImportService
{
    ...
    public PackImportService(FewshotDbContext db, IMemoryService memory, ILogger<PackImportService> log)
    {
        _db = db;
        _memory = memory;
        _log = log;
    ...
    /// <summary>
    /// Import from unencrypted FewshotPack JSON (local export or dev use).
    /// </summary>
    ...
    /// <summary>
    /// Import from encrypted .fewshotpack envelope with a decryption key.
    /// </summary>
    ...
    /// </summary>
    private async Task<PackImportResult> ImportPackAsync(FewshotPack pack, string? targetProject)
    {
