...
public class ProjectSessionService : IProjectSessionService
{
    ...
    public ProjectSessionService(FewshotDbContext db, IMemoryCache cache, ILogger<ProjectSessionService> logger)
    {
        _db = db;
        _cache = cache;
        _logger = logger;
    ...
    public async Task<(int SessionId, bool IsNew)> GetOrCreateAsync(string project)
    {
        ...
        _logger.LogInformation("Created new Fewshot session {SessionId} for project '{Project}'", session.SessionId, project);
    ...
    private static string BuildKey(string project) =>
        $"fewshot:session:{project.ToLowerInvariant()}";
}
