...
using Fewshot.Api.Jobs;
using Fewshot.Core.Interfaces;
using Fewshot.Core.Models;
using Fewshot.Infrastructure.Audit;
using Fewshot.Infrastructure.Context;
using Fewshot.Infrastructure.Data;
using Fewshot.Infrastructure.Experiments;
...
var rawConn = builder.Configuration.GetConnectionString("FewshotDb")
    ?? "Data Source=%PROGRAMDATA%\\Fewshot\\fewshot.db";
...
// ── Data ─────────────────────────────────────────────────────────
builder.Services.AddDbContext<FewshotDbContext>(options =>
...
// ── Ollama HTTP client ────────────────────────────────────────────
var ollamaUrl = builder.Configuration["Fewshot:Ollama:BaseUrl"] ?? "http://127.0.0.1:11434";
...
{
    var db = scope.ServiceProvider.GetRequiredService<FewshotDbContext>();
...
if (app.Environment.IsDevelopment() ||
    builder.Configuration.GetValue<bool>("Fewshot:EnableSwagger"))
{
    app.UseSwagger();
    app.UseSwaggerUI();
}
...
if (app.Environment.IsDevelopment() ||
    builder.Configuration.GetValue<bool>("Fewshot:EnableHangfire"))
{
    app.UseHangfireDashboard("/hangfire");
}
app.MapControllers();
app.MapHub<FewshotHub>("/hubs/fewshot");
