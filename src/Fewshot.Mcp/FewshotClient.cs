...
public class FewshotClient
{
    ...
    public FewshotClient(IHttpClientFactory factory) => _factory = factory;

    private HttpClient Http => _factory.CreateClient("Fewshot");
    ...
    public async Task<string> RemoveProjectAsync(string name)
    {
        ...
        if (match is null) return $"No project found with name '{name}'. Use fewshot_list_projects to see available names.";
    ...
    public async Task<string> GetContextAsync(int sessionId, string project, string? topic)
    {
        ...
        if (!resp.IsSuccessStatusCode)
        {
            ...
            return $"[Fewshot] Context unavailable — {resp.StatusCode}: {Truncate(detail, 500)}";
        }

        var result = await resp.Content.ReadFromJsonAsync<ContextResult>(_json);
        return result?.AssembledContext ?? "[Fewshot] Empty context returned";
    }
    ...
    public async Task<string> SearchMemoryAsync(string query, int topK = 5)
    {
        ...
        {
            var detail = await resp.Content.ReadAsStringAsync();
            return $"[Fewshot] Memory search failed — {resp.StatusCode}: {Truncate(detail, 500)}";
        }
